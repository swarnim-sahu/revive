...
def get_dashboard_summary() -> DashboardSummaryResponse:
    ...
    return DashboardSummaryResponse(
        provenance="CUSTOMER OPERATIONAL STATE (100-Customer Batch)",
...
def get_dashboard_customer_audit(customer_id: str) -> AuditTimelineResponse:
    """
    ...
    # 4. GUARD
    guard_status = "EXECUTED" if customer_rec.eligibility_status == "ELIGIBLE" else "BLOCKED"
    stages.append(
        AuditStageRecord(
            stage_index=4,
            stage_name="GUARD",
            status=guard_status,
            timestamp=customer_rec.decision_timestamp,
            summary=f"Policy Evaluation: {customer_rec.eligibility_status} under {customer_rec.policy_version}",
            details={
    ...
    completed_count = sum(1 for s in stages if s.status in {"EXECUTED", "RECOVERED", "DIRECTLY_OBSERVED", "SYNTHETIC_OBSERVED"})
...
def get_dashboard_exceptions() -> ExceptionCenterResponse:
    """
